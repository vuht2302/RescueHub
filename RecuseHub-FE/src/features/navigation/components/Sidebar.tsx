...
import { Map as MapIcon, Zap, AlertTriangle } from "lucide-react";
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const navItems: { id: View; label: string; icon: React.ElementType }[] = [
    { id: "home", label: "Ban do hien truong", icon: MapIcon },
    { id: "track", label: "Theo doi dieu phoi", icon: Zap },
    { id: "alerts", label: "Canh bao he thong", icon: AlertTriangle },
  ];
  ...
  return (
    <aside className="hidden md:flex flex-col gap-2 p-4 pt-4 bg-slate-100 dark:bg-slate-900 h-[calc(100vh-80px)] w-64 fixed left-0 border-r border-outline-variant/10">
      <div className="px-2 mb-6">
        <div className="text-lg font-bold text-slate-900 dark:text-slate-100 font-headline">
          Trung tam dieu hanh
        </div>
        <div className="text-xs text-on-surface-variant font-medium">
          Phien truc dang hoat dong
        </div>
      </div>
