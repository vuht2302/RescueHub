...
export function TeamManagementSection() {
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : "Đã xảy ra lỗi khi tải danh sách đội");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const getStatusText = (status: { code: string; name: string }) => {
    switch (status.code) {
      ...
      case "IN_USE": return "Đang làm nhiệm vụ";
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm">
      <div className="border-b border-slate-100 p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900" style={{ fontFamily: "var(--font-primary)" }}>
            Quản lý Đội Cứu Hộ
          </h2>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <p className="mt-3 text-sm text-slate-500">Đang tải danh sách đội...</p>
        </div>
      ) : (
            ...
            <tbody>
              ...
              {filteredTeams.map((team) => (
                <tr key={team.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <p className="text-sm font-semibold text-slate-800">{team.leader?.displayName || "Chưa có"}</p>
                    {team.leader?.phone && <p className="text-xs text-slate-500">{team.leader.phone}</p>}
                  </td>
                  <td className="px-6 py-4">
                    <p className="text-sm text-slate-700">
                      <span className="font-medium text-slate-900">{team.memberCount}</span> thành viên
                    </p>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      ...
                      {team.notes && (
                        <span className="text-xs text-slate-500 mt-1 truncate max-w-[200px]" title={team.notes}>
                          {team.notes}
                        </span>
                      )}
                    </div>
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-bold leading-none ${getStatusStyle(team.status.code)}`}>
                      <div className={`h-1.5 w-1.5 rounded-full ${getStatusPoint(team.status.code)}`} />
